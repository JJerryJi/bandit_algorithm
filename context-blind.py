...
def g_optimal_design(A):
    ...
    # print('np.max', np.max([np.dot(np.dot(a.T, np.linalg.inv(v_pi)), a) for a in A]))
    # print('second', (1 + .01) * n_features)
...
def ExplorationPolicy(Z, T):
    ...
    for i in range(m):
        ...
        print('W_N', W_n)
        print('Determinant', np.linalg.det(W_n))
        print('Inverse', W_n_inv)
...
delta = delta + sum([np.outer(list_y_t[i], list_y_t[i]) for i in range(math.ceil(T_tleda[0]/2))])
theta_hat = np.dot(np.linalg.inv(delta), sum([list_r_t[i] * list_y_t[i] for i in range(math.ceil(T_tleda[0]/2))]))
...
for i in range(math.ceil(T_tleda[0]/2), T_tleda[0]):
    Z_i = natural_elimination(list_A_t[i], delta, theta_hat)
    Z.append(Z_i)



# Z = np.array(Z)
# print('number of arms', len(Z))
w, l = ExplorationPolicy(Z, T)
# print(w)
# print(l)


# for k in range(1, n_arms):
    # for t in range(T_tleda[k]):
